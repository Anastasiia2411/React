...
import { HeaderIconList, HeaderIcon, CountWrapper } from "./Header.style";
import { LikeListModal } from "./LikeListModal";
import { ThemeContext, BuyContext } from "../../App";
import { BuyListModal } from "./BuyListModal";
import PropTypes from "prop-types";

export class IconList extends React.Component {
    state = { openModalB: false, openModalC: false, products: [] };

    constructor(props) {
        super(props);
        this.openModalB = this.openModalB.bind(this);
        this.closeModalB = this.closeModalB.bind(this);
        this.openModalС = this.openModalС.bind(this);
        this.closeModalС = this.closeModalС.bind(this);
    ...
    handleClickOutside(event) {
        if (this.state.openModalB === true) {
            if (this.wrapperRef && !this.wrapperRef.current.contains(event.target)) {
                this.closeModalB();
            }
        } else if (this.state.openModalC === true) {
            if (this.wrapperRef && !this.wrapperRef.current.contains(event.target)) {
                this.closeModalС();
            }
        }
    ...
    openModalС() {
        this.setState({ ...this.state, openModalC: true });
    }

    closeModalС() {
        this.setState({ ...this.state, openModalC: false });
    }
            ...
            <HeaderIconList>
                <li>
                    <a href="" onClick={(e) => {
                        ...
                        <HeaderIcon viewBox="0 0 25 24" fill="none"
                        ...
                        <CountWrapper>
                            <ThemeContext.Consumer>
                                {value => <h3>{value.length}</h3>}
                            </ThemeContext.Consumer>
                        </CountWrapper>
                    </a>
                    {this.state.openModalB &&
                        <div ref={this.wrapperRef}>
                            <LikeListModal text={"Вподобане"} type={"liked"} delEl={this.props.delEl}
                                           handleClick={this.closeModalB}/>
                        </div>
                    }
                </li>
                <li>
                    <a href="" onClick={(e) => {
                        ...
                        <HeaderIcon viewBox="0 0 25 25" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            <path
                    ...
                    {this.state.openModalC && <div ref={this.wrapperRef}>
                        <BuyListModal delEl={this.props.delEl} likedArr={this.props.likeArr} text={"Кошик"} type={"bag"}
                                      handleClick={this.closeModalС}/>
                    </div>}
                </li>
                <li>
                    <a href="">
                        <HeaderIcon viewBox="0 0 20 25" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            <path
